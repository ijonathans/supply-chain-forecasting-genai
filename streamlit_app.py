...
if uploaded_file:
    ...
    try:
        ...
        logger.info(f"Data loaded successfully: {len(df)} rows, {len(columns)} columns")
    except Exception as e:
        error_msg = str(e)
    ...
    with st.spinner("Generating forecasts..."):
        try:
            ...
            if not st.session_state.selected_group_columns or not st.session_state.selected_group_columns[0]:
                logger.info("Running single forecast (no grouping)...")
                ...
                model, forecast, fig1, fig2, plotly_fig = run_forecast(
                    agg_df, 
                    st.session_state.target_column, 
                    st.session_state.periods, 
                    st.session_state.frequency, 
                    st.session_state.data_color, 
                    st.session_state.forecast_color
                )
                ...
                if model and forecast is not None:
                    ...
                    logger.info("Single forecast results stored. Rerunning app...")
                    st.experimental_rerun()
                else:
                    logger.error("Single forecast failed: Model or forecast is None.")
                    st.error("Failed to generate single forecast. Please check your data and settings.")
            
            # Multi-group forecasting
            else:
                logger.info("Running multi-group forecast...")
                # Primary group forecasting
                logger.info("Running primary group forecast...")
                primary_fig, primary_forecasts, primary_agg_df = run_multi_group_forecast(
                    df, 
                    [st.session_state.selected_group_columns[0]], 
                    st.session_state.target_column, 
                    st.session_state.periods, 
                    st.session_state.frequency, 
                ...
                logger.info("Primary group forecast completed.")
                ...
                if len(st.session_state.selected_group_columns) >= 2:
                    logger.info("Running secondary group forecast...")
                    secondary_fig, secondary_forecasts, secondary_agg_df = run_multi_group_forecast(
                        df, 
                        [st.session_state.selected_group_columns[1]], 
                        st.session_state.target_column, 
                        st.session_state.periods, 
                        st.session_state.frequency, 
                        st.session_state.context, 
                ...
                logger.info("Combined group forecast completed.")
                results_dict['combined'] = (combined_fig, combined_forecasts, combined_agg_df)
                
                st.session_state.forecast_results = results_dict
                logger.info("Multi-group forecast results stored. Rerunning app...")
                st.experimental_rerun()
        except Exception as e:
...
if 'forecast_results' in st.session_state and st.session_state.forecast_results:
    logger.info("Displaying forecast results...")
    ...
    if selected_filter == "Overall (No Grouping)" and 'single' in results:
        logger.info("Displaying overall forecast results...")
        ...
        if not os.getenv("OPENAI_API_KEY"):
            ...
            with st.spinner("Generating business insights..."):
                logger.info("Generating business insights for overall forecast...")
        ...
        if selected_filter == f"Combined ({' & '.join(st.session_state.selected_group_columns)})" and 'combined' in results:
            logger.info("Displaying combined group forecast results...")
            ...
            if combined_forecasts:
                ...
                if f"heatmap_data_{group_title}" in st.session_state:
                    logger.info(f"Generating heatmap for {group_title}...")
                ...
                if selected_group != "All":
                    ...
                    if not os.getenv("OPENAI_API_KEY"):
                        ...
                        with st.spinner("Generating business insights..."):
                            logger.info(f"Generating business insights for {selected_group}...")
                    ...
                    logger.info(f"Running detailed forecast for {selected_group}...")
                    model, _, fig1, fig2, plotly_fig = run_forecast(
                        group_data, 
                        target_column, 
                        periods, 
                        frequency, 
                    ...
                    logger.info("Detailed forecast completed.")
                    if fig1:
                        st.pyplot(fig1)
                    if fig2:
                        st.pyplot(fig2)
            ...
            logger.info(f"Displaying primary group forecast results for {st.session_state.selected_group_columns[0]}...")
            ...
            logger.info(f"Displaying secondary group forecast results for {st.session_state.selected_group_columns[1]}...")
...
if st.session_state.forecast_results:
    if st.sidebar.button("Reset Application", key="reset_app_button"):
        logger.info("Reset Application button clicked. Resetting session state...")
        st.session_state.forecast_results = {}
        st.session_state.date_column = "Date"
        st.session_state.context = "This is a supply chain dataset with sales data across different stores and departments."
        st.session_state.target_column = "Weekly_Sales"
        ...
        st.session_state.run_button = False
        st.experimental_rerun()
        logger.info("Session state reset. App rerunning...")